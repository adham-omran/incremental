...
public class TopicTableData {
    ...
    public TopicTableData(int id, String addedDate, String scheduledDate, String viewedDate, double aFactor, double priority, String title, String pdfPath, String kind, int currentPage, int pdfPage, int parentTopic) {
        this.id = id;
        ...
        if ("extract".equals(this.kind)) {
            this.type = "Extract";
            this.source = parentTopic > 0 ? "Topic #" + parentTopic + (pdfPage > 0 ? " (p." + pdfPage + ")" : "") : "Unknown";
        } else if (pdfPath != null && !pdfPath.trim().isEmpty()) {
            this.type = "PDF";
            this.source = getFileNameFromPath(pdfPath) + (currentPage > 0 ? " (p." + currentPage + ")" : "");
        } else {
            this.type = "Image";
    ...
    public int getParentTopic() { return parentTopic; }
